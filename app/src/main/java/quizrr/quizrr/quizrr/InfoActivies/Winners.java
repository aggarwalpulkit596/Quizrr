...
import quizrr.quizrr.com.BuildConfig;
...
public class Winners extends AppCompatActivity {
    ...
    private void shareImage(File file) {
        ...
        if (position <= 3) {
            if (position == 0)
                intent.putExtra(Intent.EXTRA_TEXT, "Look, I just came " + position + 1 + "st on Quizrr and won ₹" + quiz.getPrize().get(position).getPrize() + "\n" +
                        "\n" +
                        "You can also play and win.Download Quizrr from http://bit.ly/playquizrr. To earn a free lifeline, join using my referral code \"" + QuizApp.getPreferenceManager().getString(MyPreferenceManager.USERNAME) + "\"");
            else if (position == 1)
                intent.putExtra(Intent.EXTRA_TEXT, "Look, I just came " + position + 1 + "nd on Quizrr and won ₹" + quiz.getPrize().get(position).getPrize() + "\n" +
                        "\n" +
                        "You can also play and win.Download Quizrr from http://bit.ly/playquizrr. To earn a free lifeline, join using my referral code \"" + QuizApp.getPreferenceManager().getString(MyPreferenceManager.USERNAME) + "\"");
            else if (position == 2)
                intent.putExtra(Intent.EXTRA_TEXT, "Look, I just came " + position + 1 + "rd on Quizrr and won ₹" + quiz.getPrize().get(position).getPrize() + "\n" +
                        "\n" +
                        "You can also play and win.Download Quizrr from http://bit.ly/playquizrr. To earn a free lifeline, join using my referral code \"" + QuizApp.getPreferenceManager().getString(MyPreferenceManager.USERNAME) + "\"");
        } else
